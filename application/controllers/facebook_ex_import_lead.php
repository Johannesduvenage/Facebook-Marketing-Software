...
class facebook_ex_import_lead extends Home
{
    ...
    public function user_details_modal(){
        ...
        $html .= " <div id='#alert'></div>
            ...
            <tbody>";
                $i=1;
                //print_r($one_page_user_details);die;
        foreach ($one_page_user_details as $one_user) 
        {
            $html .= "<tr id='".$i."'><td><input type='checkbox' name='checkbox[]' class='checkbox' data-id='check".$i."' value='".$one_user['id']."'></td>";
            $html .= "  <td>".$one_user['client_username']."</td>
            ...
            {
                $html .= "<button id ='".$one_user['id']."-".$one_user['permission']."' type='button' class='client_thread_subscribe_unsubscribe btn btn-default'>unsubscribe</button>";//$one_user['permission'];
            }
            ...
            $html .= "<input type='hidden' id='lead_del".$i."' value='".$one_user['id']."'>";
            $html .= "</td><td><button id='".$i."' class='btn btn-danger lead_del'>Delete</button></td>";
            $html .= "</tr>";
         $i++;
           }
        ...
        $html .= "</tbody>
                ...
                <hr>
                <button class='btn- btn-danger footer-btn' id='multi_lead_delete'>Delete</button>
                ";
    ...
    public function delete_lead()
    {
       ...
       $result = $this->basic->lead_del($user_id);
       if($result=1)
        {
          echo "<div class='alert alert-success'>Lead deleted successfully.</div>"; 
        }
        else
        {
            echo "<div class='alert alert-danger'>Unable to delete lead.</div>"; 
        }
    }
    
    public function delete_multi_lead()
    {
       if (empty($_POST['user_id'])) {
       die();
       }
        $user_id = $_POST['user_id'];
        print_r($user_id);
        $result = $this->basic->lead_multi_del($user_id);
        if($result=1)
        {
          echo "<div class='alert alert-success'>Lead deleted successfully.</div>"; 
        }
