...
class facebook_ex_campaign extends Home
{
    ...
    public function campaign_report_data()
    {
        ...
        for($i=0;$i<count($info);$i++) 
        {
            ...
            else $scheduled_at = '<i class="fa fa-close text-danger" title="Not scheduled"></i>';
            ...
            if($posting_status=='1')
            $info[$i]["delete"] = '<i class="fa fa-close text-danger" title="This campaign is in processing state"></i>';
            else $info[$i]['delete'] =  "<a title='Delete this campaign' id='".$info[$i]['id']."' class='delete btn-sm btn btn-danger' style='color:#fff;'><i class='fa fa-trash'></i> Delete</a>";
            
            if( $posting_status == '2') $info[$i]['post_status_formatted'] = '<span class="label label-success" style="padding:7px;"><i class="fa fa-check"></i> Completed</span>';
            else if( $posting_status == '1') $info[$i]['post_status_formatted'] = '<span class="label label-warning" style="padding:7px;"><i class="fa fa-spinner"></i> Processing</span>';
            else $info[$i]['post_status_formatted'] = '<span class="label label-danger"><i class="fa fa-close text-danger"></i> Pending</span>';
            
            if( $info[$i]['campaign_type'] == 'page-wise') $info[$i]['campaign_type_formatted'] = '<span class="label label-info" style="padding:7px;">Multipage</span>';
            else $info[$i]['campaign_type_formatted'] = '<span class="label label-warning" style="padding:7px;">Custom</span>';

            if($info[$i]['attached_video']!="") $info[$i]["attachment"] = "<a target='__BLANK' href='".$info[$i]['attached_video']."'><i class='fa fa-paperclip'></i></a>";
            else if($info[$i]['attached_url']!="") $info[$i]["attachment"] = "<a target='__BLANK' href='".$info[$i]['attached_url']."' style='  color: #1e1e1e;font-size: 16px;'><i class='fa fa-paperclip'></i></a>";
            else $info[$i]['attachment'] = '<i class="fa fa-close text-danger" title="Not attachemnt"></i>';
            ...
            $info[$i]['report'] =  "<a title='View campaign report' cam-id='".$info[$i]['id']."' class='sent_report btn-sm btn btn-primary' style='color:#fff;'><i class='fa fa-list'></i> Report</a>";
            
            if($posting_status!='0') $info[$i]['edit'] = '<i class="fa fa-close text-danger" title="Only pending campaigns are editable"></i>';
            else
            {
    ...
    public function campaign_sent_status()
    {
        ...
        echo '<style>.camp_name{ border-bottom: 1px solid #54667a !important;
    ...
    text-align: center;}</style>';
        ...
        $response .= "<h4 class='camp_name' ><span class='pull-left '></span>".$campaign_name."<span class='pull-right'>".$posting_status."</span></h4><div class='clearfix'></div>";
