...
class fb_msg_manager extends Home
{
    ...
    public function get_page_list()
    {
        ...
        foreach($info as $results){
            ...
            $result[$i]['page_profile'] = '<img class="img-thumbnail" src="'.$results['page_profile'].'"  style="border-radius:100%;margin-top:-21px; margin-left:-1px;height: 60px; width: 60px;padding: 5px;">';
            $result[$i]['page_name'] = $results['page_name'];
            $result[$i]['page_email'] = $results['page_email'];

            if($results['msg_manager'] == '0')
                $result[$i]['msg_manager'] = "<span table_id='".$results['id']."' action='enable' class='action label label-success' style='cursor : pointer;'><i class='fa fa-check'></i> Enable</span>";
            else
                $result[$i]['msg_manager'] = "<span table_id='".$results['id']."' action='disable' class='action label label-warning' style='cursor : pointer;'><i class='fa fa-remove'></i> Disable</span>";
    ...
    public function get_unread_message()
    {
        ...
        if(!empty($last_sync_time))
        {
            ...
            $str .= '<h3 class="red refresh_button_holder" style="margin:0px;"><div class="well text-center clearfix">Last update time : '.date("Y-m-d H:i:s",strtotime($last_sync_time)).'<button id="refresh_button" class="btn btn-success pull-right"><i class="fa fa-refresh"></i> Refresh Data</button></div></h3>';
        }



        $str .= '<div class="well text-center"><h4 class="header_title">All unread messages</h4></div>
                <script>
                    $j(document).ready(function() {
                        $("#unread_message_table").DataTable();
                    }); 
                 </script>
                 <table id="unread_message_table" class="table table-bordered table-hover table-stripped table-responsive">
                     <thead>
                         <tr>
                             <th>Thumbnail</th>
                             <th>Page Name</th>
        ...
        foreach($unread_message as $value)
        {
            ...
            $str .= '<tr>
                        ...
                        <td><p data-toggle="tooltip" title="'.$value['last_snippet'].'">'.$message_short.'</p></td>
                        <td>'.$value['from_user'].'</td>
                        <td>'.date("Y-m-d H:i:s",strtotime($last_update_time)).'</td>
                        <td>'.$value['message_count'].'</td>
                        <td>'.$value['unread_count'].'</td>
                        <td><span class="label label-success reply_button" page_name="'.$value['page_name'].'" page_table_id="'.$value['page_table_id'].'" thread_id="'.$value['thread_id'].'" style="cursor:pointer;"><i class="fa fa-mail-reply"></i> Reply</span></td>
                        <td><a class="label label-info" href="'.$finalgo_to_link.'" target="_blank"><i class="fa fa-hand-o-right"></i> Go to inbox</a></td>
                    </tr>';
        }
    ...
    public function get_pages_conversation()
    {
        ...
        if(empty($page_list))
        {
            echo '<div class="well text-center"><h3 class="header_title red">You have not enabled messenger manager for any page yet !</h3></div>';
        }
        else
        {
            $str = '';
            ...
            {
                $str .= '<div class="well text-center"><h4 class="header_title"><img class="img-thumbnail" src="'.$value['page_profile'].'" alt="image"> '.$value['page_name'].' (Last 20 conversation)</h4></div>';
                $where = array();
                $where['where'] = array(
                    'user_id' => $this->user_id,
                    'facebook_rx_fb_user_info_id' => $this->session->userdata('facebook_rx_fb_user_info'),
                ...
                else
                {
                    ...
                    foreach($last_conversation as $data)
                    {
                        ...
                        $str .= '<tr>
                                    <td><p data-toggle="tooltip" title="'.$data['last_snippet'].'">'.$message_short.'</p></td>
                                    <td>'.$data['from_user'].'</td>                                    
                                    <td>'.date("Y-m-d H:i:s",strtotime($last_update_time)).'</td>
                                    <td>'.$data['message_count'].'</td>
                                    <td><span class="label label-success reply_button" page_name="'.$value['page_name'].'" page_table_id="'.$data['page_table_id'].'" thread_id="'.$data['thread_id'].'" style="cursor:pointer;"><i class="fa fa-mail-reply"></i> Reply</span></td>
                                    <td><a class="label label-info" href="'.$finalgo_to_link.'" target="_blank"><i class="fa fa-hand-o-right"></i> Go to inbox</a></td>
                                </tr>';
                    }
                    $str .= '</tbody>
            ...
            if($value['from']['name'] == $page_name)
            {
                $str .= '<div class="clearfix">
                            <div class="right_content">
                                <p class="right_content_message">'.$value['message'].'</p>
                                <p class="right_content_name">Sent by '.$value['from']['name'].'</p>
                                <p class="right_content_date text-center">'.date('Y-m-d H:i:s',strtotime($created_time)).'</p>
                            </div>
                        </div>';
            }
            else
            {
                $str .= '<div class="clearfix">
                            <div class="left_content">
                                <p class="left_content_message">'.$value['message'].'</p>
                                <p class="left_content_name">Sent by '.$value['from']['name'].'</p>
                                <p class="left_content_date text-center">'.date('Y-m-d H:i:s',strtotime($created_time)).'</p>
                            </div>
                        </div>';
            }
        }
        if($str == '') $str = '<div class="alert alert-danger text-center"><h4>No data to show!</h4></div>';
        echo $str;
    }
    ...
    public function reply_to_conversation()
    {
        ...
        {            
            ...
            {
                echo '<div class="alert alert-success text-center">Your reply has been sent successfully!</div>';
            }
            else 
            {
