...
class facebook_rx_cta_poster extends Home
{
    ...
    public function cta_post_list_data()
    {
        ...
        for($i=0;$i<count($info);$i++) 
        {
            ...
            if( $posting_status == '2') $info[$i]['status'] = '<span class="label label-success">Completed</span>';
            else if( $posting_status == '1') $info[$i]['status'] = '<span class="label label-warning">Processing</span>';
            else $info[$i]['status'] = '<span class="label label-danger">Pending</span>';
            ...
            if($posting_status=='2')
            $post_url = "<a target='_BLANK' href='".$info[$i]['post_url']."'><span class='label label-info'><i class='fa fa-hand-o-right'></i> Visit</span></a>";
            else $post_url = '<i class="fa fa-remove red" title="This post is not published yet."></i>';
            $info[$i]['visit_post'] =  $post_url;    

            $info[$i]['delete'] =  "<a title='Delete this post from our database' id='".$info[$i]['id']."' class='delete btn-sm btn btn-danger' style='color:#fff;'><i class='fa fa-remove'></i> Delete</a>"; 
    ...
    public function add_cta_post()
    {
        ...
        $cta_dropdown = array("MESSAGE_PAGE"=>"MESSAGE_PAGE","DOWNLOAD"=>"DOWNLOAD","LEARN_MORE"=>"LEARN_MORE"
            ,"SEND_MESSAGE"=>"SEND_MESSAGE","REQUEST_TIME"=>"REQUEST_TIME","SEE_MENU"=>"SEE_MENU","SIGN_UP"=>"SIGN_UP","WATCH_MORE"=>"WATCH_MORE"
            ,"SHOP_NOW"=>"SHOP_NOW","APPLY_NOW"=>"APPLY_NOW","BOOK_NOW"=>"BOOK_NOW","CONTACT_US"=>"CONTACT_US");
    ...
    public function add_cta_post_action()
    {
        ...
        foreach ($post as $key => $value) 
        {
          ...
          // $insert_data[$key]=implode(',',$value);    
           }
        }
        unset($insert_data["schedule_type"]);
//echo "<pre>"; print_r($insert_data); die('stop execution');
        //************************************************//
        ...
        foreach ($page_info as $key => $value) 
        {
            ...
            if($schedule_type=="now")
            {
                try
                {
                    $response = $this->fb_rx_login->cta_post($message, $link,$link_description,$link_caption,$cta_type,$cta_value,$link_preview_image,"",$page_access_token,$fb_page_id);                   
                }
                catch(Exception $e) 
                {
       ...
       if($schedule_type=="now") $return_val=array("status"=>"1","message"=>"<i class='fa fa-check-circle'></i>  Facebook CTA post has been performed successfully.");
       else
       {
            if($this->db->insert_batch("facebook_rx_cta_post",$insert_data_batch))
            {
                ...
                $return_val=array("status"=>"1","message"=>"<i class='fa fa-check-circle'></i>  Facebook CTA post campaign has been created successfully.");
            }
            else $return_val=array("status"=>"0","message"=>"<i class='fa fa-remove'></i>  Something went wrong. Facebook CTA post campaign has been failed.");
       }
