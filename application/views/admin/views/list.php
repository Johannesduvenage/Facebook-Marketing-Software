...
?><div class="bDiv" style="padding:0 15px !important;">
		<table class="display nowrap table table-hover table-striped table-bordered dataTable" id="flex1" cellspacing="0" width="100%">
		<thead>
			<tr class='hDiv'>
...
<?php foreach($list as $num_row => $row){ ?>        
		<tr  <?php if($num_row % 2 == 1){?>class="erow odd"<?php }?>>
			<?php foreach($columns as $column){?>
			...
			<td align="left" width='20%'>
				<div class='tools list-pge-icon'>				
                    ...
                    <?php }?>
                    <?php if(!$unset_edit){?>
						<a href='<?php echo $row->edit_url?>' title='<?php echo $this->l('list_edit')?> <?php echo $subject?>' class="edit_button"><i class="fa fa-pencil text-inverse m-r-10"></i></a>
					<?php }?>
					<?php if(!$unset_read){?>
							...
							<a href="<?php echo $action_url; ?>" class="<?php echo $action->css_class; ?> crud-action" title="<?php echo $action->label?>"><?php 
								if(!empty($action->image_url))
								{
									?><img src="<?php echo $action->
                                                                                image_url; ?>" alt="<?php echo $action->label?>" class="key-img"/><?php 	
								}
							?></a>		
					<?php }
					}
